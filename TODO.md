需要人来做的事情：
1. 数据节点：中途退市的股票不再继续获取数据【是否存在退市后又上市的？】
2. 对于 strategy 节点，是用 T-1 的信号去交易 T日，所以还是右对齐。（后续框架只在 18:00 数据更新后调用，那么 strategy 的 window 应该置为 1.0，才能保证最后一天的信号时效性；至于回测部分，应该缓存上一日的交易计划数据，并在当前日计算净值，所以交易计划、净值计算是两条线，相互独立的）。
3. node 节点的逻辑要重构一下，有些逻辑需要收口成一个函数。现在逻辑比较长。
