...
4. mlflow 记录 best_epoch；【主要问题是树模型能不能给出 best_epoch】
5. MLP 的特征重要性计算太简陋了。可以统一换成 x 和 yhat 的 相关性 [缺点是只能度量一阶线性关系]；也可以换成微分的形式，可能需要更多的显存和时间。
6. 树模型 和 线性模型 也要加时序的滚动验证
7. 测试 残差block 与 原始 mlp 的效果对比
