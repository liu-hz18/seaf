...
3. 来一些内存优化。比如 lazy import 之类的。
4. MLP 的特征重要性计算太简陋了。可以统一换成 x 和 yhat 的 相关性 [缺点是只能度量一阶线性关系]；也可以换成微分的形式，可能需要更多的显存和时间。
5. 树模型 和 线性模型 也要加时序的滚动验证
6. 测试 残差block 与 原始 mlp 的效果对比
7. model_mlp 节点的内存一直在上升，在 train samples 从 30w 上涨到 37w 的过程中，内存从4500MB 涨到了 31100 MB，需要排查。是否有无用变量的内存或显存没有释放的问题。【但是主动运行一次windows系统的清理内存助手之后又大幅降低了】
8. strategy 节点的耗时不断增加，从 340步 时 4s 左右增加到 1600步时113s 左右，这个节点主要处理每日的股票分组和交易，按照交易次数来计的话，耗时不会增长很多。需要排查耗时不断增长的问题。
9. 存在一些一直在持仓中，卖不出去的股票，需要排查
10. snapshot node input csv 占据了很大空间[窗口较长]，尝试采用 zip 压缩来处理 input 文件。同时降低 input 的采样率。
11. 因为显存充足，batch_size, model 可以更大一些【先不计算策略节点，以提高效率】
9. 风控：策略节点的分组内，增加 1/volality 的仓位权重，表示风险平价的权重分配.
10. 下载接口在晚上9点之后变得很快，单日单股票数据约 1s 即可下载完成
