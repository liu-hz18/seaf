需要人来做的事情：
1. lgbm 模型 IC性能下降的原因研究，经过排查 precision 参数，发现是价格精度下降导致的，如何补偿这部分精度损失 ?
...
mlp 的 signal 要比 lgbm 的更加标准正态，偏度和极端值更小，变化也更平稳。
2. seafquant\model_wrappers.py中，扩展或自定义 lgbm 模型或 ridge 模型，三种模型要支持 pearson ic 损失函数, 也就是对于单批次的样本给出的预测值 (Batch size, feature) -> (Batch size, ) 。计算其与 真实值 (Batch size,) 的 -Pearson ic（取负值表示最大化）。pearson ic 是一个序列损失函数而不是MSE一样的单点损失函数。基于torch的神经网络因为自动求导机制，这个功能将很好实现。但是对于 lgbm 和 ridge 机器学习器，可能需要结合api自定义一些模块或工具。
