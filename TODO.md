需要人来做的事情：
1. 开展回测
2. node.py 中增加 mlflow 对 buffer size 和 两个游标 的记录
3. MLP 的特征重要性计算太简陋了。可以统一换成 x 和 yhat 的 相关性 [缺点是只能度量一阶线性关系]；也可以换成微分的形式，可能需要更多的显存和时间。
4. 树模型 和 线性模型 也要加时序的滚动验证
5. 测试 残差block 与 原始 mlp 的效果对比
6. model_mlp 节点的内存一直在上升，在 train samples 从 30w 上涨到 37w 的过程中，内存从4500MB 涨到了 31100 MB，需要排查。是否有无用变量的内存或显存没有释放的问题。【但是主动运行一次windows系统的清理内存助手之后又大幅降低了】
7. 存在一些一直在持仓中，卖不出去的股票，需要排查
8.  风控：策略节点的分组内，增加 1/volality 的仓位权重，表示风险平价的权重分配.
9.  下载接口在晚上9点之后变得很快，单日单股票数据约 1s 即可下载完成
10. idx=588[2009-06-05]-604 的 label skew 和 kurt 非常高 > 5.0; kurt > 10, 并且是跳变, 需要排查: 解答：是因为 raw_ret 有部分股票的 max 非常大，但是我们在训练时已经做了 clip，所以影响不大。
11. 增加简单风控模块：strategy 节点前置增加一个计算 Y-Z 波动率的因子，ewm 移动平均 或 简单平均【这是调参的部分】，然后波动率属性接入下游 strategy 节点，节点做 波动率反比 的权重分配。【我们的策略是最大化全截面超额收益夏普，做风控本质是在绝对收益夏普与超额收益夏普之间做权衡】
12. IC 节点增加一个 window 内 信号 自相关性 的计算 mean(个股信号实践序列的自相关性)
13. dump 每日 pred_signal 向量，用于独立进行风控的研究。数据包括每个日期股票的 close, close_uq, tradestatus, isST 和 pred_signal。基线是每日 top 10% 的股票。这部分还是希望交给 AI 来迭代调参。注意时间穿越的问题。
...
1. model_node.py 和 ic_analysis.py 中的节点逻辑函数返回值需要相应修改
...
排查策略节点 position_value 先上升后下降最后又接近0的问题 [解释：是资金不足或者分组不够细致的原因，如果分10组，截面上是5000支股票，每只持仓10000元以上，分散持仓20天，则每组至少需要 20*10000*5000/10 = 1_0000_0000 = 1亿元的资金]
