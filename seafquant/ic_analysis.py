...
- 来自 data source 的 pipeline：close 数据，window=21 天（计算 20 日 fwd_ret）
...
def ic_analysis_fn(name: str, f3d: Frame3D, context: Any) -> Tuple[Frame3D, Any]:
    ...
    # 计算 20 日 fwd_ret：使用最早时间的 close（t-20 时刻的 close vs t 时刻的 close）
    # 实际上我们需要 close[t+20]/close[t] - 1，但在流式框架中，
    # 我们可以用 close[latest] / close[earliest_within_window] - 1 
    # 其中 earliest 是 20 天前（如果 window=21）
    # 更准确的做法：用 t-20 天的 close 和 t 天的 close
    if len(times) >= 21:
        t_20 = times[-21]  # 20 天前
        cs_mask_20 = df.index.get_level_values('key') == t_20
        close_now = df.loc[cs_mask, 'close'].values
        close_then = df.loc[cs_mask_20, 'close'].values
        ...
        if valid_mask.sum() >= 10:
            ...
            if fwd_std > 0:
                fwd_xd = (fwd_valid - fwd_mean) / fwd_std
            else:
                fwd_xd = np.zeros_like(fwd_valid)
