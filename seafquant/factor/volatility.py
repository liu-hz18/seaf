...
def compute_volatility_factors(name: str, f3d: Frame3D, context) -> Frame3D:
    """计算 33 个波动率+日内因子。"""
    result = f3d.copy()
    close, high, low, open_p, volume = (f3d.df['close'], f3d.df['high'],
                                         f3d.df['low'], f3d.df['open'],
                                         f3d.df['volume'])
    df = result.df
    ...
    df['factor_intra_composite'] = (
        ...
        df['factor_intra_hl_range_5d'] + df['factor_intra_gap_efficiency_5d']) / 4

    # 联合截面标准化
    factor_cols = [c for c in df.columns
                   if c.startswith('factor_vol_') or c.startswith('factor_intra_')]
