...
def compute_quality_pattern_factors(name: str, idx: int, f3d: Frame3D, context) -> Frame3D:
    """计算 9 个质量形态/高级因子。"""
    result = f3d.copy()
    high, low, _close = f3d.df['high'], f3d.df['low'], f3d.df['close']
    ...
    def _up_down_ratio_vec(series, window):
        ...
        pos_mean = np.where(pos_cnt > 0, (win * pos_mask).sum(axis=1) / np.maximum(pos_cnt, 1), 0.0)
        neg_mean = np.where(
            neg_cnt > 0, np.abs((win * neg_mask).sum(axis=1)) / np.maximum(neg_cnt, 1), 1e-6
        )
