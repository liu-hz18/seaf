...
def compute_quality_autocorr_factors(name: str, f3d: Frame3D, context) -> Frame3D:
    ...
    df['factor_qa_autocorr_60d'] = df.groupby('code')['_ret'].transform(
        lambda x: x.rolling(60, min_periods=20).corr(x.shift(1))
    )
    df['factor_qa_autocorr_20d'] = df.groupby('code')['_ret'].transform(
        lambda x: x.rolling(20, min_periods=10).corr(x.shift(1))
    )

    df['factor_qa_tail_risk_60d'] = df.groupby('code')['_ret'].transform(
        lambda x: -x.where(x < 0).rolling(60, min_periods=20).quantile(0.05)
    )
    df['factor_qa_tail_risk_120d'] = df.groupby('code')['_ret'].transform(
    ...
    logging.debug(
        f'QA-Autocorr NaN: { {c: result.df[c].isna().sum() for c in factor_cols} }'
    )
