...
def compute_counting_factors(name: str, f3d: Frame3D, context) -> Frame3D:
    ...
    ret_pivot.iloc[:, :] = up; df['factor_cnt_consec_up_10d'] = ret_pivot.stack()
    ret_pivot.iloc[:, :] = down; df['factor_cnt_consec_down_10d'] = ret_pivot.stack()
    ret_pivot.iloc[:, :] = rp20; df['factor_cnt_run_pct_20d'] = ret_pivot.stack()
    ret_pivot.iloc[:, :] = rp60; df['factor_cnt_run_pct_60d'] = ret_pivot.stack()
    ...
    df['factor_cnt_composite'] = (
        ...
        df['factor_cnt_vol_shrink_20d'] / 20) / 3

    def _new_high_count(series, window):
        arr = series.values.astype(float); n = len(arr)
        if n < max(2, window // 2): return np.full(n, np.nan)
        clean = np.where(np.isnan(arr), -np.inf, arr)
        cmax = np.maximum.accumulate(clean)
        is_nh = np.zeros(n, dtype=np.float64)
        for i in range(1, n):
            if arr[i] > cmax[i - 1] and cmax[i - 1] != -np.inf: is_nh[i] = 1.0
        win = sliding_window_view(is_nh, window); out = np.full(n, np.nan)
        out[window - 1:] = win.sum(axis=1); return out
    ...
    def _new_low_count(series, window):
        ...
        for i in range(1, n):
            if arr[i] < cmin[i - 1] and cmin[i - 1] != np.inf: is_nl[i] = 1.0
        win = sliding_window_view(is_nl, window); out = np.full(n, np.nan)
        out[window - 1:] = win.sum(axis=1); return out

    result = result.add_column('factor_cnt_new_high_20d',
        close.groupby(grp).transform(lambda x: _new_high_count(x, 20)), cp=False)
    result = result.add_column('factor_cnt_new_high_60d',
        close.groupby(grp).transform(lambda x: _new_high_count(x, 60)), cp=False)
    result = result.add_column('factor_cnt_new_low_60d',
        close.groupby(grp).transform(lambda x: _new_low_count(x, 60)), cp=False)
